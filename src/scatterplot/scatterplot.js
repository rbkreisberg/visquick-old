        ...
        if (regress==='linear') {
            var    sum_x = pv.sum(valid_data, function(d) {
                ...
                //sum_y2 = pv.sum(valid_data, function(d) { return d[y] * d[y]; }),
                    ...
                    slope = ((data_len * sum_xy) - (sum_x * sum_y)) / ((data_len * sum_x2) - (sum_x * sum_x));
                    intercept = (sum_y - slope * sum_x) / data_len;
        }
        else if(regress==='median') {
            ...
            for (var i =0; i < 3; i++) {
                ...
                medians[i] = {
                                x:pv.median(regions[i], function(r) { return r[x];}), 
                                y:pv.median(regions[i], function(r) { return r[y];})
                            };
            }
